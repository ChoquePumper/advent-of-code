...
local GridMap; GridMap = {
	...
	foldLeft = function(self, x_val)
		...
		--assert((math.abs(self.min_x)+math.abs(self.max_x)+1) / 2 == x_val)
...
function solvePart1(input_iterable)
	...
	for line in input_iterable do
		...
		--table.insert(list_dots, {x=x,y=y})
		map:set(x,y, "#")
	end
	-- input_iterable returned nil or empty line here. Now parse the fold instructions
	...
	for i,fold in ipairs(fold_instructions) do
		...
		break -- for now, focus on just completing the first fold.
	end
	--map:printMap()
	return map:countVisibleDots()
end
...
local function stringLineIterator(text)
	...
	return function()
		...
		if found_at then
			...
			if #line<1 or not line:find("(%S)") then line = nil end
			next_i = found_at+1
		else
